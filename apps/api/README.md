...
Все маршруты требуют ролей `admin` или `moderator` и используют те же JWT httpOnly cookies.

- `GET /api/admin/listings` — очередь объявлений с фильтрами `q`, `status`, `dealerId`, `cityId`, `limit`, `offset`.
- `GET /api/admin/listings/:id` — детальная карточка объявления для модерации (любой статус).
- `POST /api/admin/listings/:id/publish` — публикация объявления.
- `POST /api/admin/listings/:id/reject` — отклонение с указанием `reason`.
- `POST /api/admin/listings/bulk` — массовая модерация (`action=publish|reject`, массив `ids`).
- `GET /api/admin/stats/overview` — агрегированные метрики (количество объявлений, дилеров, специалистов, активные промо + серия за 7 дней).
